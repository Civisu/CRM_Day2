...
<BODY>
<FORM id=form1 name=form1 action="${pageContext.request.contextPath }/customerAction_save.action" method=post>
            ...
            <TD vAlign=top width="100%" bgColor=#ffffff>
                ...
                <TABLE cellSpacing=0 cellPadding=5 border=0>
                    <TR>
                        ...
                        <td>所属行业 ：</td>
                        <td>
                            <s:select list="#industryList" name="industry.dictId" listKey="dictId"
                    ...
                    <TR>
                        ...
                        <td>
                            <%--<select name="custSource" class=textbox id=sChannel2 style="WIDTH: 180px">
                                <c:forEach items="${sourceList}" var="dict">
                                    <option value="${dict.dictId}">${dict.dictItemName}</option>
                                </c:forEach>
                            </select>--%>
                            <%--使用Struts2框架提供标签库的下拉选择框的标签,显示数据
                                <s:select list="要遍历的集合,OGNL表达式" name="" headerKey="请选择的option对应的值" headerValue="-请选择-" listKey="dictId" listValue=""/>
                                <s:select list="#sourceList" name="source.dictId" listKey="dictId" listValue="dictItemName"/>--%>
                            <s:select list="#sourceList" name="source.dictId" listKey="dictId"
                                      listValue="dictItemName"/>
                        </td>
                        <td>客户级别：</td>
                        <td>
                            <s:select list="#levelList" name="level.dictId" listKey="dictId"
                                      listValue="dictItemName"/>
                        </td>
                    </TR>
                    <TR>
